...
class Fuel_page extends Fuel_base_library {
    ...
    public function variables_render($return = FALSE, $fuelify = FALSE) {
        ...
        if (empty($output) && empty($vars['allow_empty_content']))
            return FALSE;
    ...
    public function render_marker($key) {
        ...
        if ($this->fuel->config('admin_enabled') AND
                is_fuelified() AND ( is_null($this->CI->load->get_var('fuelified')) OR $this->CI->load->get_var('fuelified') === TRUE)
    ...
    public function properties($prop = NULL) {
        if (is_string($prop) AND isset($this->_page_data[$prop]))
            return $this->_page_data[$prop];
    ...
    public function variables($key = NULL) {
        if (is_string($key) AND isset($this->_variables[$key]))
    ...
    public function segment($n = NULL) {
        if (is_int($n) AND isset($this->_segments[$n]))
    ...
    public function find_view_file($view, $depth = NULL) {
        ...
        if (!$this->fuel->config('auto_search_views') AND empty($depth))
            return NULL;
