...
class Base_module_model extends MY_Model {
    ...
    public function archive($ref_id, $data) {
        ...
        foreach ($remove_from_check as $val) {
            if (!empty($last_archive_data[$val]))
                unset($last_archive_data[$val]);
            if (!empty($tmp_data[$val]))
                unset($tmp_data[$val]);
        }
...
class Base_module_record extends Data_record {
    ...
    public function after_get($output, $var) {
        if (is_string($output)) {
            ...
            if ($parsed_fields === TRUE OR ( is_array($parsed_fields) AND in_array($var, $parsed_fields))) {
